...
function WebServer(func) {
    this.handle = 
        ...
        func : (function(request, query) "Not Found: " + request)
}
...
WebServer.parseQuery = function(queryString) {
    return Arrays.asList(queryString.split("&"))
       .stream()
       .map(function(s) s.split("=", 2))
       .collect(groupingBy(function(a) WebServer.decode(a[0]),
          mapping(function(p) WebServer.decode(p.length > 1? p[1] : ""),
            toList())))
}
...
// main
WebServer.create()
    .bind("/hello", function() "Hello World")
    .bind("/echo", function(path, query) query.toString())
